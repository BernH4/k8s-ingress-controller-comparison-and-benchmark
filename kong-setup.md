...
kubectl apply -n kong -f kong/quickstart/gateway.yml
kubectl apply -n kong -f kong/quickstart/gatewayclass.yml
```
...
kubectl apply -f kong/quickstart/httproute.yml
```
...
```sh
helm upgrade kong kong/ingress -n kong -f kong/production/values.yml
```
...
kubectl delete -f kong/quickstart/httproute.yml --ignore-not-found=true
kubectl delete -f kong/quickstart/gateway.yml -n kong --ignore-not-found=true
```
...
```sh
kubectl apply -f kong/production/gateway.yml
