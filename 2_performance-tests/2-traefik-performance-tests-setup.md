...
- HTTPS setup according to `/traefik/2-traefik-https.md`

### Install Traefik

Deploy Traefik with some minor changes compared to DeveloperExperience Tests:
...
While the script is running, execute the loadtest on the VM with the following parameters, it will send 10k RPS and will just hit the single endpoint configured.
...
Traefik was installed using Helmchart version 37.4.0, which installed the Traefik v3.6.2
Test Rolling Updates by downgrading to Helmchart version to 37.2.0, which will downgrade Traefik to v3.5.3:
